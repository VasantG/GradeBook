...
assignment_avgs = [0 for i in gradebook[0]]
...
for i in range(len(gradebook)):
    ...
    for j in range(len(gradebook[i])):
        ...
        assignment_avgs[j] += gradebook[i][j]

        student_total += gradebook[i][j]

    student_avgs.append(student_total/len(gradebook[i]))

print("Assignment Averages:")

for i in range(len(assignment_avgs)):

    assignment_avgs[i] /= len(gradebook)

    print("Assignment "+str(i+1)+": %.2f"%assignment_avgs[i])

print("Student Averages:")

for i in range(len(student_avgs)):

    print("Student "+str(i+1)+": %.2f"%student_avgs[i])
